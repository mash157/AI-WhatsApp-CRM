...
import { Code, Mail, Lock, LogIn, Sun, Moon } from 'lucide-react';
...
const DeveloperLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await authAPI.developerLogin(formData);
    ...
    <div className='min-h-screen bg-white dark:bg-gray-950 flex items-center justify-center p-4 transition-colors duration-300'>
      ...
      <div className='crm-card w-full max-w-md bg-white dark:bg-gray-900 shadow-lg dark:shadow-2xl transition-colors duration-300'>
        <div className='text-center mb-8'>
          <h2 className='text-2xl font-bold text-gray-900 dark:text-white transition-colors duration-300'>Developer Portal</h2>
          <p className='text-gray-500 dark:text-gray-400 mt-2 transition-colors duration-300'>Sign in for API access and testing</p>
        </div>
        
        <form onSubmit={handleSubmit} className='space-y-6'>
          <div>
            <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors duration-300'>Developer Email</label>
            <div className='relative'>
              <div className='absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none'>
                <Mail className='h-5 w-5 text-gray-400 dark:text-gray-500 transition-colors duration-300' />
              </div>
              <input type='email' className='crm-input pl-10 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 transition-colors duration-300' placeholder='dev@company.com'
          ...
          <button type='submit' disabled={loading} className='crm-button w-full flex justify-center items-center py-2 bg-gray-900 dark:bg-gray-700 hover:bg-black dark:hover:bg-gray-600 focus:ring-gray-900 dark:focus:ring-gray-700 transition-colors duration-300'>
            {loading ? 'Authenticating...' : <><Code className='w-5 h-5 mr-2' /> Developer Sign In</>}
          </button>
        </form>

        <div className='mt-6 text-center text-sm'>
          <span className='text-gray-500 dark:text-gray-400 transition-colors duration-300'>Need API access? </span>
          <Link to='/developer-register' className='text-gray-900 dark:text-white hover:text-black dark:hover:text-gray-100 font-medium transition-colors duration-300'>Register here</Link>
        </div>
        
        <div className='mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 text-center transition-colors duration-300'>
          <Link to='/login' className='inline-flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 transition-colors duration-300'>
            <LogIn className='w-4 h-4 mr-2' /> Back to Standard Login
          </Link>
        </div>
      </div>
    </div>
